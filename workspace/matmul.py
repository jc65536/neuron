# Example code from
# https://awsdocs-neuron.readthedocs-hosted.com/en/latest/general/nki/tutorials/matrix_multiplication.html
...
def nki_matmul_basic_(lhsT: NDArray[T], rhs: NDArray[T]) -> NDArray[T]:
...
@nki.jit(mode="simulation")
def nki_matmul_tiled_(lhsT: NDArray[T], rhs: NDArray[T]) -> NDArray[T]:
    ...
    for m in nl.affine_range(M // TILE_M):
        for n in nl.affine_range(N // TILE_N):
            ...
            for k in nl.affine_range(K // TILE_K):
                ...
                res_psum += nl.matmul(lhsT_tile[...],
                                      rhs_tile[...], transpose_x=True)
...
@nki.jit(mode="simulation")
def nki_matmul_hoist_load_(lhsT: NDArray[T], rhs: NDArray[T]) -> NDArray[T]:
    """NKI kernel to compute a matrix multiplication operation in a tiled manner
    ...
    for m in nl.affine_range(M // TILE_M):
        ...
        for n in nl.affine_range(N // TILE_N):
            ...
            # Copy the result from PSUM back to SBUF, and cast to expected output data-type
            res_sb = nl.copy(res_psum, dtype=result.dtype)
            nl.store(result[m * TILE_M + i_res.p, n *
                     TILE_N + i_res.x], value=res_sb)

    return result


@nki.jit(mode="simulation")
def nki_matmul_block_free_dimension_(lhsT: NDArray[T], rhs: NDArray[T]) -> NDArray[T]:
                ...
                for bn in nl.affine_range(TILES_IN_BLOCK_N):
                    for bm in nl.affine_range(TILES_IN_BLOCK_M):
                        ...
                        for bk in nl.affine_range(TILES_IN_BLOCK_K):
                            res_tile[...] += nl.matmul(
                                lhsT_tiles[bk, i_lhsT_mm.p,
                                           bm * TILE_M + i_lhsT_mm.x],
                                rhs_tiles[bk, i_rhs_mm.p,
                                          bn * TILE_N + i_rhs_mm.x],
                                transpose_x=True)
...
def main():
    ...
    a = np.random.rand(2048, 1024).astype(np.float16)
    ...
    def check_match(nki_func):
        ...
        rel_error = abs(diff / c_true).max()
        print(f"Max rel error: {rel_error}")

        allclose = np.allclose(c_true, c, atol=1e-4, rtol=1e-2)
